...
def convert_place(place_type, place):
    title = ''
    text = ''

    for i in range(0, 4):
        key = '{}_{}'.format(place_type, i)

        if key in place:
            if isinstance(place[key], list) and len(place[key]) > 0:

                to_remove = [
                    'Det finns mycket att göra där (ange exempel nedan)',
                    'Annat'
                ]
                ...
                if title == '' and len(place[key]) > 0:
                    title = place[key][0]

                text += templates[i][place_type].format(''.join(
                    ['<li>{}</li>'.format(a) for a in place[key]]))
            elif isinstance(place[key], str) and place[key] != "":
                if title == '':
                    title = place[key]

                text += templates[i][place_type].format(place[key])
    ...
    return {
        ...
        'category': {
            'id': category[place_type]
        },
        'address': '{}, {}'.format(place['lat'], place['lng']),
        'latitude': place['lat'],
        'longitude': place['lng'],
    }
